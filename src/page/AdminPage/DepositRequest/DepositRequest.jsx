...
const DepositRequest = () => {
  ...
  const handleSearch = async () => {
    try {
      if (searchTerm) {
        const res = await api.post(`/deposit-requests/search?email=${user?.email}`, {
          searchTerm,
        });
        setData(res.data);
      } else {
        const res = await api.get(`/deposit-requests?email=${user?.email}`);
        setData(res?.data);
      }
    } catch (error) {
      console.error(error);
  ...
  useEffect(() => {
    api.get(`/deposit-requests?email=${user?.email}`).then((res) => {
  ...
  const handleApproved = () => {
    ...
    api
      .put(`/approved/deposit/${viewDetails?._id}?email=${user?.email}`)
      .then((response) => {
        console.log(" successfully.", response?.data);
      })
      .catch((error) => {
