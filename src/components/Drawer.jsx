...
const Drawer = ({ menuItems,subOfferMenuItem }) => {
      ...
      <List className="space-y-2">
        {menuItems.map((item, index) => (
          <div key={index}>
            {item !== "Offers" ? (
              <NavLink
                to={`/admin_dashboard/${item.toLowerCase()}`}
                className={`block px-4 py-3 rounded-md hover:bg-green-100 font-semibold ${
                  isNavLinkActive(`/admin_dashboard/${item.toLowerCase()}`) ? "text-green-400" : ""
                }`}
              >
                {item}
              </NavLink>
                ...
                {isOpen && (
                  <div className="pl-6 mt-2">
                    <NavLink
                      ...
                      className={`block px-4 py-3 rounded-md hover:bg-green-100 font-semibold ${
                        isNavLinkActive("/admin_dashboard/offer/airtel") ? "text-green-400" : ""
                      }`}
                    >
                      Airtel
                    </NavLink>
                    <NavLink
                      ...
                      className={`block px-4 py-3 rounded-md hover:bg-green-100 font-semibold ${
                        isNavLinkActive("/admin_dashboard/offer/robi") ? "text-green-400" : ""
                      }`}
                    >
                      Robi
                    </NavLink>
